...
public class ApiAudioAdTest {
    ...
    public void deserializesAudioAd() throws Exception {
        ...
        expect(audioAd.trackingImpressionUrls).toEqual(Lists.newArrayList("https://promoted.soundcloud.com/impression?adData=instance%3Asoundcloud%3Bad_id%3A82%3Bview_key%3A1401441272728730%3Bzone_id%3A6&loc=&listenerId=38cfbf01fa3450d214e1cc37548d18&sessionId=7a9cb38dc0b93f35a2c1ee36de75c54b&ip=%3A%3Affff%3A62.72.64.50&user_agent=http.async.client%2F0.5.2&cbs=4354567"));
        expect(audioAd.trackingFinishUrls).toEqual(Lists.newArrayList("https://promoted.soundcloud.com/track?reqType=SCSoundFinished&protocolVersion=2.0&adId=82&zoneId=6&cb=c34180f2e5d7433f8357879e355a58e0"));
        expect(audioAd.trackingSkipUrls).toEqual(Lists.newArrayList("https://promoted.soundcloud.com/track?reqType=SCSoundSkipped&protocolVersion=2.0&adId=82&zoneId=6&cb=882f90b95de847f9bfa8684943cf761f"));
        expect(audioAd.getUrn()).toEqual("adswizz:ads:263");
        expect(audioAd.getCompanion().urn).toEqual("adswizz:ads:954");
        expect(audioAd.getLeaveBehind().urn).toEqual("adswizz:ads:1105");
    ...
    public void shouldResolveToPropertySet() throws CreateModelException {
        ...
        expect(propertySet.contains(AdProperty.AD_AUDIO_URN)).toBeTrue();
        expect(propertySet.contains(AdProperty.AD_VISUAL_URN)).toBeTrue();
