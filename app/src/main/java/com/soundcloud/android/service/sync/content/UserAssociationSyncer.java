...
public class UserAssociationSyncer extends SyncStrategy {
    ...
    public ApiSyncResult syncContent(@NotNull Uri uri, @Nullable String action) throws IOException {
        return syncUserAssociations(Content.match(uri), SoundCloudApplication.getUserId());
    }

    private ApiSyncResult syncUserAssociations(Content content, final long userId) throws IOException {
        ...
        if (content == Content.ME_FOLLOWINGS && userAssociationStorage.hasFollowingsNeedingSync()) {
            for (UserAssociation userAssociation : userAssociationStorage.getFollowingsNeedingSync()) {
    ...
    // TODO I realize this has no business being here. Waiting on other PRs to find a home
    private boolean pushUserAssociation(UserAssociation a) {
        final Request request = Request.to(Endpoints.MY_FOLLOWING, a.getUser().id);
        try {
            int status = (a.isMarkedForAddition() ? mApi.put(request) : mApi.delete(request)).getStatusLine().getStatusCode();
            final boolean success;
            if (a.isMarkedForAddition()) {
