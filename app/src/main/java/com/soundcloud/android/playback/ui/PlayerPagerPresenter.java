...
public class PlayerPagerPresenter extends DefaultSupportFragmentLightCycle<PlayerFragment>
    ...
    private static final Func2<AdData, PropertySet, PlayerItem> TO_PLAYER_AD = new Func2<AdData, PropertySet, PlayerItem>() {
        ...
        public PlayerItem call(AdData adData, PropertySet trackProperties) {
            if (adData instanceof VideoAd) {
                ...
                return new AudioPlayerAd((AudioAd) adData, trackProperties);
            }
        }
    };
    ...
    private Observable<? extends PlayerItem> getStationObservable(final PlayQueueItem playQueueItem)  {
        return Observable.zip(
                getTrackObservable(playQueueItem.getUrn(), playQueueItem.getAdData()).map(toPlayerTrackState(playQueueItem)),
