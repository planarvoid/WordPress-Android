...
constructor(private val eventBus: EventBusV2) {
    ...
    fun report(audioPerformanceEvent: AudioPerformanceEvent) {
    ...
    private fun createPlaybackPerformanceEvent(event: AudioPerformanceEvent): PlaybackPerformanceEvent {
        val builder: PlaybackPerformanceEvent.Builder = when (event.metric) {
            ...
            else -> throw IllegalArgumentException("Unexpected performance metric : $event")
        }

        return populatePlaybackPerformanceBuilder(builder, event)
    }

    protected fun populatePlaybackPerformanceBuilder(builder: PlaybackPerformanceEvent.Builder,
                                                     event: AudioPerformanceEvent): PlaybackPerformanceEvent {
        return builder
