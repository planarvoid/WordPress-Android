...
public class RemoteCollectionLoader<T extends ScModel> extends CollectionLoader<T> {
    ...
    public ReturnData<T> load(AndroidCloudAPI app, CollectionParams<T> params) {
        try {
            ...
            CollectionHolder<T> holder = ScModel.getCollectionFromStream(resp.getEntity().getContent(),
                    app.getMapper(),
                    params.loadModel);
