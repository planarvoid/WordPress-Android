...
public class LegacyCastSessionControllerTest extends AndroidUnitTest {
    ...
    public void onConnectedToReceiverAppExpandsPlayerWhenLocalQueueIsPopulated() throws Exception {
        ...
        when(playSessionStateProvider.getLastProgressForItem(URN)).thenReturn(new PlaybackProgress(123L, 456L, URN));
    ...
    public void onMetaDataUpdatedSetsPlayQueueWithDifferentTracklist() throws Exception {
        castSessionController.startListening();
        when(castOperations.loadRemotePlayQueue()).thenReturn(RemotePlayQueue.create(PLAY_QUEUE, URN));
