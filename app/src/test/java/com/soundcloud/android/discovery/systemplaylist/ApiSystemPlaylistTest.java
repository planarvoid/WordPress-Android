...
public class ApiSystemPlaylistTest {
    ...
    private static final String JSON = "{\"urn\": \"" + URN + "\",\"track_count\": " + TRACK_COUNT + ",\"last_updated\": \"" + LAST_UPDATED + "\",\"title\": \"" + TITLE + "\",\"description\": \"" + DESCRIPTION + "\",\"artwork_url_template\": \"" + ARTWORK_URL + "\",\"tracks\": " + TRACKS_JSON + "}";

    private final JsonTransformer jsonTransformer = new JacksonJsonTransformer();

    @Test
    public void deserialize() throws Exception {
        ApiSystemPlaylist EXPECTED = ApiSystemPlaylist.create(new Urn(URN), Optional.of(TRACK_COUNT), Optional.of(new ApiDateFormat().parse(LAST_UPDATED)), Optional.of(TITLE), Optional.of(DESCRIPTION), Optional.of(ARTWORK_URL), new ModelCollection<>());
