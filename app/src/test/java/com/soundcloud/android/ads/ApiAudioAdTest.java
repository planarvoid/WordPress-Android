...
public class ApiAudioAdTest {
    ...
    public void deserializesAudioAd() throws Exception {
        ApiAudioAd apiAudioAd = TestHelper.getObjectMapper().readValue(
                getClass().getResourceAsStream("audio_ad.json"), ApiAudioAd.class);

        expect(apiAudioAd.getApiTrack().getUrn()).toEqual(Urn.forTrack(96416915));
        ...
        expect(apiAudioAd.getTrackingFinishUrls()).toEqual(Lists.newArrayList("https://promoted.soundcloud.com/track?reqType=SCSoundFinished&protocolVersion=2.0&adId=82&zoneId=6&cb=c34180f2e5d7433f8357879e355a58e0"));
        expect(apiAudioAd.getTrackingSkipUrls()).toEqual(Lists.newArrayList("https://promoted.soundcloud.com/track?reqType=SCSoundSkipped&protocolVersion=2.0&adId=82&zoneId=6&cb=882f90b95de847f9bfa8684943cf761f"));
        expect(apiAudioAd.getApiLeaveBehind().getUrn()).toEqual("adswizz:ads:1105");
        expect(apiAudioAd.getApiLeaveBehind().getImageUrl()).toEqual("https://va.sndcdn.com/mlb/sqsp-example-leave-behind.jpg");
        expect(apiAudioAd.getApiLeaveBehind().getClickthroughUrl()).toEqual("http://squarespace.com");
        expect(apiAudioAd.getApiLeaveBehind().getTrackingImpressionUrls()).toContain("https://promoted.soundcloud.com/impression?adData=instance%3Asoundcloud%3Bad_id%3A1105%3Bview_key%3A1410853892331806%3Bzone_id%3A56&loc=&listenerId=5284047f4ffb4e04824a2fd1d1f0cd62&sessionId=67fa476869b956676b5bae2866c377a9&ip=%3A%3Affff%3A80.82.202.196&OAGEO=ZGUlN0MxNiU3Q2JlcmxpbiU3QzEwMTE1JTdDNTIuNTMxOTk3NjgwNjY0MDYlN0MxMy4zOTIxOTY2NTUyNzM0MzglN0MlN0MlN0MlN0MlM0ElM0FmZmZmJTNBODAuODIuMjAyLjE5NiU3Q3RoZSt1bmJlbGlldmFibGUrbWFjaGluZStjb21wYW55K2dtYmg=&user_agent=SoundCloud-Android%2F14.09.02+%28Android+4.3%3B+Genymotion+Sony+Xperia+Z+-+4.3+-+API+18+-+1080x1920%29&cbs=681405");
        expect(apiAudioAd.getApiLeaveBehind().getTrackingClickUrls()).toContain("https://promoted.soundcloud.com/track?reqType=SCAdClicked&protocolVersion=2.0&adId=1105&zoneId=56&cb=dfd1b6e0c90745e9934f9d35b174ff30");
    }

    @Test
    public void shouldResolveToPropertySet() throws CreateModelException {
        ApiAudioAd apiAudioAd = ModelFixtures.create(ApiAudioAd.class);

        final PropertySet propertySet = apiAudioAd.toPropertySet();
