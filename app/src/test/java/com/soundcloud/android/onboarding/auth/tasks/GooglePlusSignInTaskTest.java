...
public class GooglePlusSignInTaskTest extends AndroidUnitTest {
    ...
    public void shouldTryToLoginWithTheCorrectParameters() throws IOException, GoogleAuthException {
        ...
        when(signInOperations.signIn(bundleArgumentCaptor.capture())).thenReturn(AuthTaskResult.success(new AuthResponse(new Token("validtoken", null), Me.createFromUserRecord(user, configuration, false)), SignupVia.GOOGLE_PLUS));
    ...
    public void shouldReturnSuccessIfGoogleSignInWasSuccessful() throws IOException, GoogleAuthException, ApiMapperException, ApiRequestException {
        when(apiClient.fetchMappedResponse(argThat(isApiRequestTo("GET", ApiEndpoints.ME.path())), isA(TypeToken.class))).thenReturn(Me.createFromUserRecord(user, configuration, false));
        when(accountOperations.getGoogleAccountToken(eq(ACCOUNT_NAME), eq(SCOPE), any(Bundle.class))).thenReturn("validtoken");
        when(app.addUserAccountAndEnableSync(eq(user), any(Token.class), any(SignupVia.class))).thenReturn(true);
        when(signInOperations.signIn(any())).thenReturn(AuthTaskResult.success(new AuthResponse(token, Me.createFromUserRecord(user, configuration, false)), SignupVia.GOOGLE_PLUS));
