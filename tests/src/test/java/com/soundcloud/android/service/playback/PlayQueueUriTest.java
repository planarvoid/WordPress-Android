...
public class PlayQueueUriTest {
    ...
    public void shouldBuildUri() throws Exception {
        ...
        expect(playQueueUri.toUri(new Track(123l), 0, 100l, playSourceInfo).toString())
    ...
    public void shouldBuildUriWithParams() throws Exception {
        PlayQueueUri playQueueUri = new PlayQueueUri(new PlayQueueUri(Content.ME_SOUND_STREAM.uri).toUri(new Track(123l), 1, 200l, playSourceInfo));
        expect(playQueueUri.getPos()).toEqual(1);
        expect(playQueueUri.getSeekPos()).toEqual(200);
        expect(playQueueUri.getTrackId()).toEqual(123);
        expect(playQueueUri.getTrackingInfo()).toEqual(playSourceInfo);
    }

    @Test
    public void shouldOverwriteOldParams() throws Exception {
        Uri oldUri = new PlayQueueUri(Content.ME_SOUND_STREAM.uri).toUri(new Track(123l), 1, 200l, playSourceInfo);
        Uri newUri = new PlayQueueUri(oldUri).toUri(new Track(456l), 2, 400l, playSourceInfo2);
