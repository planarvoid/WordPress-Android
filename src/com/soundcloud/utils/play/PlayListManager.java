	...
	public Track getTrackAt(int pos){
		if (mPlayList != null && mPlayListLen > pos){
			if (mPlayListCache != null){
				...
				return CloudUtils.resolveTrackById((SoundCloudApplication) mPlaybackService.getApplication(), mPlayList[pos], CloudUtils.getCurrentUserId(mPlaybackService));
			}
		} else return null;
	...
	public void loadCachedPlaylist(List<Parcelable> playlistCache, int playPos){
		...
		for (int i = 0; i < playlistCache.size(); i++){
			if (playlistCache.get(i) instanceof Track){
				...
				mPlayListCache[i] = (Track) ((Event) playlistCache.get(i)).getTrack();
			}
		...
		new CommitPlaylistTask((SoundCloudApplication) mPlaybackService.getApplication(),CloudUtils.getCurrentUserId(mPlaybackService),mPlayList).execute(mPlayListCache);	
	} 
	...
	private void ensurePlayListCapacity(int size) {
	 ...
	 private final char hexdigits [] = new char [] {
	    ...
	    public void reloadQueue() {
	        String q = PreferenceManager.getDefaultSharedPreferences(mPlaybackService).getString("queue", "");
	        ...
	        if (qlen > 1) {
	            ...
	            int pos = PreferenceManager.getDefaultSharedPreferences(mPlaybackService).getInt("curpos", 0);
