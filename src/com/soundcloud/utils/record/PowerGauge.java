...
import com.soundcloud.android.SoundCloudApplication.RecordListener;
...
public class PowerGauge extends View {
    ...
    private float scalingFactor = (float) 1;
...
//			}
	...
	public void updateAmplitude(float maxAmplitude){
		...
		c.drawLine( nextBufferX, this.getHeight()/2-maxAmplitude*this.getHeight()/2, nextBufferX,  this.getHeight()/2+maxAmplitude*this.getHeight()/2, mPaint );
		 nextBufferX++;
...
*/
	 ...
	 * 
	 * 
	 private void drawSamples(byte[] buffer){
	 
	 while( i < buffer.length/2 )
		{
			
			float concentratedSample = 0; 
            try
            {
            	for (int k = 0; k < SAMPLE_CONCENTRATION; k++){
            		float sample = 0; 
            		for( int j = 0; j < ScCreate.REC_CHANNELS; j++ )
                    {
                            int shortValue = getShort(buffer[i*2], buffer[i*2+1]);
                            sample += (shortValue * MAX_VALUE);
                            i++;
                    }
                    concentratedSample += sample/ScCreate.REC_CHANNELS;
            	}
            	
            	sampleValue = concentratedSample/SAMPLE_CONCENTRATION;
            	
                if (sampleIndex == 0)
                	lastValue = createPlotValue(sampleValue);
                else {
                        float value = createPlotValue(sampleValue);
                        c.drawLine( nextBufferX + (int)((sampleIndex-1) / SAMPLES_PER_PIXEL), this.getHeight() - (int)lastValue,nextBufferX +  (int) (sampleIndex) / SAMPLES_PER_PIXEL, this.getHeight() - (int)value, mPaint );
                        lastValue = value;
                }
                sampleIndex++;
            }
		 ...
		 nextBufferX = nextBufferX + sampleIndex/SAMPLES_PER_PIXEL;
		 
		 }
		 
		 *	
	private float createPlotValue(float sampleValue){
		return (sampleValue / scalingFactor) * this.getHeight() / 3 + this.getHeight() / 2 - offset * this.getHeight() / 2;
	}
		 *
		 *
		 */
