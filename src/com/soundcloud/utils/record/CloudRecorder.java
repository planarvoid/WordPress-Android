...
public class CloudRecorder
{
		...
		public void onPeriodicNotification(AudioRecord recorder)
		{
			...
			{ 
		        ...
		        if (service != null){
		        	...
		        	//application.onFrameUpdate(((float)Math.sqrt(maxAmplitude))/MAX_ADJUSTED_AMPLITUDE);
		        	service.onRecordFrameUpdate(((float)maxAmplitude)/MAX_AMPLITUDE);
		        }
			...
			{
				e.printStackTrace();
				Log.e(CloudRecorder.class.getName(), "Error occured in updateListener, recording is aborted : ");
				stop();
			}
		}
			...
			int audioFormat)
	{
		try
		{
			...
			{ // RECORDING_UNCOMPRESSED
				...
				{ // Check to make sure buffer size is not smaller than the smallest allowed one 
					...
					Log.w(CloudRecorder.class.getName(), "Increasing buffer size to " + Integer.toString(bufferSize));
				}
		...
		} catch (Exception e)
		{
			...
			{
				Log.e(CloudRecorder.class.getName(), "Unknown error occured while initializing recording");
			}
			state = State.ERROR;
		}
	}
	...
	public void setOutputFile(String argPath)
	{
		...
		catch (Exception e)
		{
			...
			{
				Log.e(CloudRecorder.class.getName(), "Unknown error occured while setting output path");
			}
			state = State.ERROR;
		}
	}
		...
		try
		{
			if (state == State.INITIALIZING)
			{
				if (rUncompressed)
				{
					...
					{
						Log.e(CloudRecorder.class.getName(), "prepare() method called on uninitialized recorder");
						state = State.ERROR;
					}
				}
				else
	...
	public void release()
	{
		...
		else
		{
			if ((state == State.READY) & (rUncompressed))
			{
				...
				{
					Log.e(CloudRecorder.class.getName(), "I/O exception occured while closing output file");
				}
		...
		public void handleMessage(Message msg) {
			...
			case REFRESH:
				...
				if (service != null){
					...
					//max amplitude returns false 0's sometimes, so just use the last value. It is usually only for a frame
					if (mCurrentMax == 0){
					...
					//hack for not having a proper median. using a square root normalizes the amplitude and makes a better looking wave representation
		        	//application.onFrameUpdate(((float)Math.sqrt(mCurrentMax))/MAX_ADJUSTED_AMPLITUDE);
					service.onRecordFrameUpdate(((float)mCurrentMax)/MAX_AMPLITUDE);
		        }
				...
				if (mLastRefresh == 0){
					...
					long newDelay = TIMER_INTERVAL + TIMER_INTERVAL - (System.currentTimeMillis() - mLastRefresh);
					mLastRefresh = System.currentTimeMillis();
					next = newDelay;
				}
