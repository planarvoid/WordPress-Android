...
public class WaveformHolder extends RelativeLayout {
    ...
    public Matrix setMatrix(Matrix m) {
        ...
        Log.i(TAG, "Set Matrix  transform " + mTransformX + "|" + initialXScale + "|" + values[2]
                + "|" + ((RelativeLayout) this.getParent()).getWidth());
        values[2] = Math.max(
                -(this.getWidth() * values[0] - ((WaveformController) ((RelativeLayout) this
                        .getParent()).getParent()).getWidth()), Math.min(0, values[2]));
    ...
    public Matrix checkMatrix(Matrix m) {
        ...
        mTransformX = checkMatrixValues[0] = Math.min(1, Math.max(initialXScale,
                checkMatrixValues[0]));
        checkMatrixValues[2] = Math
                .max(
                        -(this.getWidth() * checkMatrixValues[0] - ((WaveformController) ((RelativeLayout) this
                                .getParent()).getParent()).getWidth()), Math.min(0,
                                checkMatrixValues[2]));
    ...
    //		
    // if (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() ==
    ...
    // Log.i("MARKER", "Get Hit Rec " + hitRect.left + " " + hitRect.width() +
    // " " + x);
