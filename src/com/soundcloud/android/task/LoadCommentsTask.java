...
public class LoadCommentsTask extends AsyncTask<String, Parcelable, Boolean> {
    ...
    protected Boolean doInBackground(String... params) {
        ...
        while (!mFinished) {
            ...
            builder.appendQueryParameter("offset", String.valueOf(commentsPerPage
                    * (commentsPageIndex)));
            ...
            try {
                ...
                try {
                    ...
                    if (CloudUtils.getErrorFromJSONResponse(jsonRaw) != "") {
                        // TODO
                        // activity.setError(CloudCommunicator.getErrorFromJSONResponse(jsonRaw));
                        return false;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                for (int i = 0; i < collection.length(); i++) {
                    ...
                    /*
                     * try { //publishProgress(new
                     * Comment(collection.getJSONObject(i))); } catch
                     * (JSONException e) { Log.i("COMMENTS",e.toString()); }
                     */
                }
