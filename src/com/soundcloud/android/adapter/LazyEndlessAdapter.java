 ...
 */
	...
	public class LazyEndlessAdapter extends AdapterWrapper {
		 ...
		 * @param wrapped : the adapter list-backing adapter that this adapter is being wrapped around
		 */
		 ...
		 * @param wrapped : the adapter list-backing adapter that this adapter is being wrapped around
		 * @param url : the base url that this adapter will receive data from
		 * @param loadModel : what type of data this adapter will receive, for parsing purposes
		 */
		...
		* @param activity : context
		 ...
		 * @param url : the base url that this adapter will receive data from
		 * @param loadModel : what type of data this adapter will receive, for parsing purposes
		 * @param collectionKey : if the return data is not an array, this key in the object will hold the array
		 */
		...
		/**
		 * Create an empty view for the list this adapter will control. This is done here because this adapter
		 * will control the visibility of the list
		 * @param lv
		 */
		...
		public void setEmptyviewText(){
			...
			switch (getLoadModel()){
				case track:
					textToSet = mException == false ? mActivity.getResources().getString(R.string.tracklist_empty) : mActivity.getResources().getString(R.string.tracklist_error);
					break;
				case user:
					textToSet = mException == false ? mActivity.getResources().getString(R.string.userlist_empty) : mActivity.getResources().getString(R.string.userlist_error);
					break;
				case comment:
					textToSet = mException == false ? mActivity.getResources().getString(R.string.tracklist_empty) : mActivity.getResources().getString(R.string.commentslist_error);
					break;
				case event:
					textToSet = mException == false ? mActivity.getResources().getString(R.string.tracklist_empty) : mActivity.getResources().getString(R.string.tracklist_error);
					break;
		...
		/**
		 * Restore a possibily still running task that could have been passed in on creation
		 * @param ap
		 */
		 ...
		 * @return an integer list {whether to keep retrieving data, the current page the adapter is on}
		 */
		...
		/**
		 * Get the current number of items in this adapter, accounting for a loading view
		 */
		@Override
		public int getCount() {
			if (keepOnAppending.get()) {
	    ...
	    */
		...
		/**
		 * We just got the data, so if there is any data that needs to be captured from it, do so
		 * This will be overridden by extending classes as necessary ({ @link com.soundcloud.android.adapter.EventsAdapter })
		 ...
		 * Set the url for this adapter
		 * @param url : url this adapter will use to get data from
		 * @param query : if this adapter is performing a search, this is the user's search query
		 */
		...
		public class AppendTask extends AsyncTask<HttpUriRequest, Parcelable, Boolean> {
			 ...
			 * context is destroyed and created in response to orientation changes
			...
			/**
			 * Add all new items that have been retrieved, now that we are back on a UI thread
			 */
			@Override
