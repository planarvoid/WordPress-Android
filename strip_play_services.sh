...
PLAY_SERVICES_FILENAME="google-play-services.aar"
...
PLAY_SERVICES_OUTPUT_FILE="google-play-services-STRIPPED.aar"

# Check if file exists in the same directory
if [ ! -f $PLAY_SERVICES_FILENAME ]; then
	echo "\nPlease put this script in the Play Services AAR location, then run it again\n\n"
	exit -1
fi
...
jar xvf classes.jar > /dev/null
rm $PLAY_SERVICES_FILENAME 
echo "Extracted.\n"
...
rm -rf com
jar cf $PLAY_SERVICES_OUTPUT_FILE *
