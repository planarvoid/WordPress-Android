 ...
 **Checkout Flow**
 ...
 - Check that it passes and then log in manually with `scandsubscribe` to confirm Go+ content, features & upsell status

**Reset test user status**

 - Go to http://sonar.soundcloud.com/users/scandsubscribe/purchases
 - Click "Terminate" under "Staging payment history" (requires payments role - alternatively see reset command in LastPass)
...
**To test the Chromecast**
  ...
  - Find a monitor that has an HDMI input (preferably also an audio output jack)
  ...
  - Connect your Chromecast and test devices to the same network
  - Cast it up!
- - -
