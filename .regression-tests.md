...
Some things have to tested manually (release branch devDebug):
Using a free-tier user account, like (`andtestpl`), please check that:
...
Now, using the same user as the acceptance tests (`android_ad_user`):
