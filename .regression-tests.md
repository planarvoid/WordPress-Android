...
# Push notifications
Localytics can send push notifications to a specific build type, but cannot send push notifications to a specific user.
Therefore, to avoid sending push notifications to our entire beta community, we must test using the debug build
rather than the beta build.

## Steps for testing

  0. If you do not have it already, get permission from another team member as an `Owner` for the `SoundCloud Android Dev` app in Localytics.
  1. In your terminal, checkout the `release` branch
  2. Build and deploy the `release` branch to your physical device
      * _Note: (Notifications will only be received if the device has Google Play Services)_
  3. Open up Localytics Dashboard
  4. **SWITCH TO THE SoundCloud Android Dev App**
  5. Go to Marketing > Messages using the sidebar navigation
  6. Duplicate the previous Regression Testing Message
  7. Follow the Wizard, and send a message immediately

## Acceptance Criteria

- [ ] As a user, when the message is sent from localytics, I receive a push notification.
