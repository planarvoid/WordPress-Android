...
- [ ] Own Profile
- [ ] Another profile with all buckets (`spotlight`, `tracks`, `albums`, `playlists`, `reposts` and `likes`)
- [ ] Bucket is hidden if no content is available (like no `albums` section if user has not marked any playlist as a `album`)
...
- [ ] `View All Albums` opens a new `Activity` listing all `posted` albums and all actions on that activity should work
